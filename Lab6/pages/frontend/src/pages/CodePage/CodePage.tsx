...
const CodePage = ({selectedCode, setSelectedCode, isMock, setIsMock}: Props) => {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`http://localhost:8000/api/codes/${id}`)
        ...
        <Container>
            <Row>
                ...
                <Col md="6">
                    ...
                    <p className="fs-5" style={{wordBreak: "break-all"}}>Информационный вектор: {selectedCode.description}</p>
                    <p className="fs-5">Объем: {selectedCode.weight} байт</p>
                </Col>
            </Row>
        </Container>
    );
